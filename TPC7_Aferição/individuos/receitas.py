livro = '''2020/001,Despesa,2020-01-04,7.28,CGD,Manutenção da conta
...
receitas = livro.split('\n')

for r in receitas:
    info = r.split(',')
    print(':Rec' + info[0].split('/')[1] + ''' rdf:type owl:NamedIndividual ,
                        :Receita ;
                :receitaDe :''' + info[4].replace(" ", "").replace("º", "") + ''' ;
    ...
    if ' ' not in info[4]:
        print(':' + info[4] +''' rdf:type owl:NamedIndividual ,
                :Fornecedor .''')
